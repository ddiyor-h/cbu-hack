"""
Проверка модели V2 на точность используя sklearn метрики
...
print("="*80)
print("ПРОВЕРКА ТОЧНОСТИ МОДЕЛИ V2 - КОМПЛЕКСНАЯ ВАЛИДАЦИЯ")
print("="*80)
...
# Загружаем обученную модель V2
model_v2 = xgb.Booster()
model_v2.load_model('/home/dr/cbu/xgboost_model_v2.json')
print("✅ Модель XGBoost V2 загружена")

# Загружаем список выбранных признаков
selected_features = pd.read_csv('/home/dr/cbu/selected_features_v2.csv')['feature'].tolist()
print(f"✅ Загружено {len(selected_features)} признаков")

# Загружаем тестовые данные
X_test_full = pd.read_parquet('/home/dr/cbu/X_test.parquet')
...
X_train_full = pd.read_parquet('/home/dr/cbu/X_train.parquet')
...
ax1.text(0.60, 0.20, textstr, transform=ax1.transAxes, fontsize=11,
...
plt.tight_layout()
plt.savefig('/home/dr/cbu/model_v2_validation_roc_curve.png', dpi=150, bbox_inches='tight')
print("✅ ROC-кривая сохранена: model_v2_validation_roc_curve.png")
plt.close()
...
# Используем параметры из обученной модели V2
model_cv = XGBClassifier(
    objective='binary:logistic',
    eval_metric='auc',
    n_estimators=20,  # Из обучения V2
    max_depth=6,
    learning_rate=0.1,
    subsample=0.8,
...
# Проверяем наличие временных признаков в выбранных признаках
time_features = [col for col in selected_features if 'year' in col.lower() or
                 'month' in col.lower() or 'day' in col.lower() or
                 'hour' in col.lower()]

print(f"\n🔍 Найдено временных признаков в выбранных: {len(time_features)}")
if len(time_features) > 0:
    print(f"   Признаки: {', '.join(time_features)}")

# Проверяем наличие account_open_year в полном датасете
if 'account_open_year' in X_train_full.columns:
    print("\n⏰ Выполняется Out-of-Time Validation по признаку 'account_open_year'...")

    # Объединяем данные (используем выбранные признаки + account_open_year для сортировки)
    X_train_with_year = X_train_full[selected_features + ['account_open_year']].copy()
    X_test_with_year = X_test_full[selected_features + ['account_open_year']].copy()

    df_full = pd.concat([
        pd.concat([X_train_with_year, pd.DataFrame({'default': y_train})], axis=1),
        pd.concat([X_test_with_year, pd.DataFrame({'default': y_test})], axis=1)
    ], axis=0)
    ...
    # Удаляем account_open_year перед обучением (используем только выбранные признаки)
    X_train_time = df_train_time[selected_features]
    y_train_time = df_train_time['default'].values
    X_test_time = df_test_time[selected_features]
    y_test_time = df_test_time['default'].values

    print(f"   • Train period: {df_train_time['account_open_year'].min():.0f} - {df_train_time['account_open_year'].max():.0f}")
    print(f"   • Test period:  {df_test_time['account_open_year'].min():.0f} - {df_test_time['account_open_year'].max():.0f}")
...
print("\n" + "="*80)
print("📊 ИТОГОВЫЙ ОТЧЕТ ПО ТОЧНОСТИ МОДЕЛИ V2")
print("="*80)
...
results_df.to_csv('/home/dr/cbu/model_v2_validation_summary.csv', index=False)

print("\n", results_df.to_string(index=False))

print(f"\n✅ Качество модели V2: {quality}")
...
print("\n" + "="*80)
print("✅ ВАЛИДАЦИЯ МОДЕЛИ V2 ЗАВЕРШЕНА!")
print("="*80)

print("\n📁 Созданные файлы:")
print("   1. model_v2_validation_roc_curve.png    - ROC-кривая и распределения")
print("   2. model_v2_validation_summary.csv      - Итоговые метрики")
